...
const interviewController = {
  ...
  // PATCH /api/recruitment/interviews/:id/result
  async updateResult(req, res, next) {
    try {
      ...
      if (!result) {
        return res.status(400).json({
          message: "Kết quả phỏng vấn là bắt buộc",
        });
      }

      await db.query(
        "UPDATE interviews SET result = ?, notes = ?, status = ? WHERE id = ?",
        [result, notes || null, "completed", req.params.id],
      );
