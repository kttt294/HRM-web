...
export function PayrollListPage() {
    ...
    const fetchPayrolls = async () => {
        setLoading(true);
        try {
            const result = await payrollApi.getAll({ month: monthFilter, year: yearFilter });
        ...
        <div ref={containerRef}>
            ...
            <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
                <div style={{
                    ...
                    <div style={{ fontSize: '14px', opacity: 0.9, marginBottom: '4px' }}>Tổng lương tháng {monthFilter}</div>
                    <div style={{ fontSize: '28px', fontWeight: '700' }}>{formatCurrency(totalNetSalary)}</div>
                </div>
                <div style={{
                    padding: '16px 24px',
                    background: 'white',
                    ...
                    <div style={{ fontSize: '24px', fontWeight: '700', color: '#4caf50' }}>{paidCount}</div>
                    <div style={{ fontSize: '13px', color: '#757575' }}>Đã thanh toán</div>
                </div>
                <div style={{
                    padding: '16px 24px',
                    background: 'white',
                    ...
                    <div style={{ fontSize: '24px', fontWeight: '700', color: '#1976d2' }}>{payrolls.length}</div>
                    <div style={{ fontSize: '13px', color: '#757575' }}>Tổng nhân viên</div>
                </div>
            </div>
                            ...
                            {payrolls.map((payroll) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td style={{ padding: '14px 16px', textAlign: 'right', color: '#4caf50', fontFamily: 'monospace' }}>
                                            +{formatCurrency(totalAllowances)}
                                        </td>
                                        <td style={{ padding: '14px 16px', textAlign: 'right', color: '#f44336', fontFamily: 'monospace' }}>
        ...
        >
            <div style={{
                ...
                <div style={{ marginBottom: '20px', padding: '16px', background: '#f5f5f5', borderRadius: '12px' }}>
                    <div style={{ fontWeight: '600', fontSize: '18px' }}>{payroll.employeeName}</div>
                    <div style={{ color: '#757575', fontSize: '14px' }}>{payroll.department}</div>
                    <div style={{ color: '#757575', fontSize: '14px' }}>Tháng {payroll.month}/{payroll.year}</div>
                </div>

                <div style={{ marginBottom: '16px' }}>
                    <h4 style={{ margin: '0 0 12px', color: '#757575', fontSize: '13px', textTransform: 'uppercase' }}>
                        Thu nhập
                    </h4>
                ...
                <div style={{ marginBottom: '16px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <DetailRow label="Bảo hiểm" value={`-${formatCurrency(payroll.deductions.insurance)}`} color="#f44336" />
                        <DetailRow label="Thuế TNCN" value={`-${formatCurrency(payroll.deductions.tax)}`} color="#f44336" />
                    </div>
                </div>
                ...
                <div style={{ 
                    ...
                    <span style={{ fontSize: '24px', fontWeight: '700' }}>{formatCurrency(payroll.netSalary)}</span>
                </div>
            </div>
        </div>
    );
